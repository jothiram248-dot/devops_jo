...
useEffect(() => {
  ...
  // If user came to dashboard asking to open "nominee",
  // enforce: feature active → Aadhaar verified → at least 1 contact
  if (id === "nominee") {
    // 1) feature must be unlocked (paid or trial)
    if (!isFeatureActive("nominee")) {
      // send to pricing section on Choose Nominee page
      navigate("/choose-nominee");
      setTimeout(() => {
        const el = document.getElementById("cyn");
        if (el) el.scrollIntoView({ behavior: "smooth", block: "start" });
      }, 400);
      return;
    }

    // 2) Aadhaar must be verified
    if (!isAadhaarVerified) {
      setShowAadhaarModal(true);
      return;
    }

    // 3) Must have at least one emergency contact
    if ((contactCount || 0) === 0) {
      setShowEmergencyContactModal(true);
      return;
    }

    // All good → open nominee
    setActiveFeature("nominee");
    return;
  }

  // For any other feature id, just open it (existing rules still apply in FeatureCard)
  setActiveFeature(id);
}, [
  id,
  isAadhaarVerified,
  contactCount,
  navigate,
  // if you have these booleans in scope, include them so the effect re-evaluates when they change:
  nomineeIsPaid,
  nomineeIsTrialActive,
]);
