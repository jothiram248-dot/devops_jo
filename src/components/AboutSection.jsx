...
const CardCarousel = () => {
  ...
  const cards = [
    {
      ...
      description: "Built with an understanding of the emotional value of digital memories.",
    ...
    },
    {
      ...
      description: "Developed based on real needs and feedback from our community.",
  ...
  return (
    <div className="relative w-full">
      {/* Fixed height container to ensure stable layout */}
      <div className="relative flex items-center justify-center w-full h-72">
        {cards.map((card, index) => {
          ...
          if (position === 0) { // Active card
            ...
            opacity = 1;
            zIndex = 30;
          } else if (position === 1) { // Card to the right
            xPosition = 340; // Increased spacing
            zIndex = 20;
            opacity = 0.7; // Slightly more visible
          } else if (position === cards.length - 1) { // Card to the left
            xPosition = -340; // Increased spacing
            zIndex = 20;
            opacity = 0.7; // Slightly more visible
          }
          ...
          return (
            <motion.div
              key={card.title}
              className={`absolute p-6 w-72 ${card.overlay} bg-gradient-to-br ${card.gradient} backdrop-blur-sm rounded-xl border-2 ${card.borderColor} shadow-xl ${isActive ? card.hoverShadow : ''} transition-all duration-500 group cursor-pointer overflow-hidden`}
              initial={{ x: position === 0 ? 0 : position === 1 ? 340 : -340, opacity: position === 0 ? 1 : 0.7, scale: position === 0 ? 1 : 0.8 }}
              animate={{
                x: xPosition,
                scale: scale,
                opacity: opacity,
            ...
            >
              ...
              <div className={`absolute -top-6 -left-6 w-12 h-12 bg-gradient-to-br ${card.cornerGlow} blur-lg opacity-0 group-hover:opacity-100 transition-opacity duration-700`}></div>
              
              {/* Top-right corner accent */}
              <div className="absolute top-0 right-0 w-16 h-16 overflow-hidden">
                <div className={`absolute top-0 right-0 w-4 h-4 bg-gradient-to-br ${card.iconGradient} rotate-45 transform origin-top-right shadow-lg`}></div>
              </div>
              
              {/* Bottom-right corner glow */}
              <div className={`absolute -bottom-6 -right-6 w-12 h-12 bg-gradient-to-tl ${card.cornerGlow} blur-lg opacity-0 group-hover:opacity-100 transition-opacity duration-700`}></div>
              
              {/* Premium metallic accent in bottom-left */}
              ...
              <div className="flex items-center space-x-4 mb-4 relative z-10">
                <div className={`w-12 h-12 flex items-center justify-center rounded-full bg-gradient-to-br ${card.iconGradient} shadow-lg ${isActive ? 'group-hover:shadow-lg' : ''} transition-all duration-300`}>
              ...
              {isActive && (
                <div className={`w-0 group-hover:w-full h-0.5 mt-6 bg-gradient-to-r ${card.activeIndicator} transition-all duration-500 relative z-10`}></div>
              )}
...
const AboutSection = () => {
  ...
  // Sparkle elements for premium effect
  const sparkles = Array(40).fill().map((_, i) => ({
    id: i,
  ...
  return (
    <>
      ...
      <section className="relative py-16 overflow-hidden bg-gradient-to-b from-gray-50 via-white to-gray-50" id="about">
        ...
        <div className="container mx-auto px-6 relative z-10">
          ...
          <div className="grid md:grid-cols-2 gap-16 items-center mb-20">
            <motion.div
              ...
              className="relative"
            >
              {/* Premium image with decorative elements */}
            ...
            >
              ...
              <p className="text-gray-300 mb-6 leading-relaxed">
                ...
                with technology. From precious memories stored on social media to
                critical financial information in banking apps and beyond, our
                digital footprint has become a major part of our daily lives.
                SacredSecret emerged from the recognition that these digital
                assets require the same level of protection and careful succession
                planning as physical assets. We've created a platform that ensures
                your digital assets remain secure and are only accessible to those
         ...
         {/* Premium Feature Cards with original styling but reduced bottom space */}
         <motion.div
...
>
  {[
    {
      ...
      description: "Your digital legacy deserves the highest level of protection.",
      items: [
    ...
    },
    {
      ...
      description: "Available anywhere, anytime, for everyone who needs it.",
      items: [
    ...
    },
    {
      ...
      description: "Constantly evolving to meet tomorrow's challenges.",
      items: [
  ...
  ].map(({ icon: Icon, color, title, image, description, items, gradient, overlay, borderColor, hoverShadow, iconGradient, cornerGlow, patternColor }, index) => (
    <motion.div
      key={title}
    ...
    >
      ...
      <div className="absolute inset-0 z-0">
        ...
        />
        <div className={`absolute inset-0 bg-gradient-to-t ${gradient} backdrop-blur-sm`}></div>
        <div className={`absolute inset-0 bg-[linear-gradient(45deg,${patternColor}_25%,transparent_25%,transparent_50%,${patternColor}_50%,${patternColor}_75%,transparent_75%)] bg-[length:20px_20px] opacity-5`}></div>
      </div>
      
      {/* Premium corner effects */}
      {/* Top-left corner glow */}
      <div className={`absolute -top-6 -left-6 w-12 h-12 bg-gradient-to-br ${cornerGlow} blur-lg opacity-0 group-hover:opacity-100 transition-opacity duration-700`}></div>
      
      {/* Top-right corner accent */}
      <div className="absolute top-0 right-0 w-16 h-16 overflow-hidden">
        <div className={`absolute top-0 right-0 w-4 h-4 bg-gradient-to-br ${iconGradient} rotate-45 transform origin-top-right shadow-lg`}></div>
      </div>
      
      {/* Bottom-right corner glow */}
      <div className={`absolute -bottom-6 -right-6 w-12 h-12 bg-gradient-to-tl ${cornerGlow} blur-lg opacity-0 group-hover:opacity-100 transition-opacity duration-700`}></div>
      ...
      {/* Premium floating sparkle elements */}
      <div className={`absolute top-1/3 right-1/3 w-6 h-6 rounded-full bg-${color}-500/20 blur-md opacity-0 group-hover:opacity-70 transition-all duration-1000 animate-pulse`}></div>
      <div className={`absolute bottom-1/4 left-1/4 w-4 h-4 rounded-full bg-${color}-500/20 blur-md opacity-0 group-hover:opacity-70 transition-all duration-1000 animate-pulse`} style={{ animationDelay: "0.5s" }}></div>
      
      {/* Premium Content Container */}
      <div className="relative z-10 h-full flex flex-col p-8 pb-4">
        {/* Header with elegant icon */}
        <div className="flex items-center space-x-4 mb-6">
          <div className={`w-14 h-14 flex items-center justify-center rounded-full bg-gradient-to-br ${iconGradient} shadow-lg group-hover:shadow-lg transition-all duration-300`}>
          ...
          <ul className="space-y-4">
            {items.map((item, idx) => (
              <li key={idx} className="flex items-start space-x-3 group/item">
                <div className={`mt-2 flex-shrink-0 w-2 h-2 rounded-full bg-gradient-to-r ${iconGradient} group-hover/item:scale-125 transition-all duration-300`}></div>
        ...
        <div className={`absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r ${iconGradient} transform translate-y-0 opacity-50 group-hover:opacity-100 transition-all duration-300`}></div>
      </div>
    </motion.div>
  ))}
</motion.div>
